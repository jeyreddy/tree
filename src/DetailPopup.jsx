  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* Add whichever parent slot in this person's birth unit is still empty.
            fatherId = partner_a, motherId = partner_b of their birth family_unit. */}
        {(!person.fatherId || !person.motherId) ? (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, marginBottom: 4 }}>
            {!person.fatherId && <button className="btn btn-gold btn-sm" onClick={() => onAdd('ancestor', 'M')}>↑ {REL?.father || 'Father'}</button>}
            {!person.motherId && <button className="btn btn-gold btn-sm" onClick={() => onAdd('ancestor', 'F')}>↑ {REL?.mother || 'Mother'}</button>}
          </div>
        ) : (
